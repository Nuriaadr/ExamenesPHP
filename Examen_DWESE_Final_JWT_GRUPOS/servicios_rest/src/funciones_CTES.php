...
function horarioProfesor($id_usuario)
{
    ...
    try {
        $consulta = "select horario_lectivo.*, grupos.nombre as grupo, grupos.id_grupo from horario_lectivo join grupos on horario_lectivo.grupo = grupos.id_grupo where usuario=?";
        $sentencia = $conexion->prepare($consulta);
...
function borrarGrupo($dia, $hora, $id_profesor, $id_grupo)
{
    try {
        $conexion = new PDO("mysql:host=" . SERVIDOR_BD . ";dbname=" . NOMBRE_BD, USUARIO_BD, CLAVE_BD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    } catch (PDOException $e) {
    ...
    try {
        ...
        $sentencia = $conexion->prepare($consulta);
        $sentencia->execute([$dia, $hora, $id_profesor, $id_grupo]);
    } catch (PDOException $e) {
        $sentencia = null;
        $conexion = null;
        $respuesta["error"] = "No he podido realizarse la consulta: " . $e->getMessage();
...
function grupos(){
    ...
    if ($sentencia->rowCount() > 0) {
        ...
        $respuesta["mensaje"] = "No hay grupos en la BD";
...
function agregarGrupo($dia, $hora, $id_profesor, $id_grupo){
    try {
        $conexion = new PDO("mysql:host=" . SERVIDOR_BD . ";dbname=" . NOMBRE_BD, USUARIO_BD, CLAVE_BD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    } catch (PDOException $e) {
        $respuesta["error"] = "No he podido conectarse a la base de batos: " . $e->getMessage();
    ...
    try {
        $consulta = "insert into horario_lectivo (dia, hora, usuario, grupo, aula) values (?, ?, ?, ?, 1)";
        $sentencia = $conexion->prepare($consulta);
        $sentencia->execute([$dia, $hora, $id_profesor, $id_grupo]);
    } catch (PDOException $e) {
        $sentencia = null;
    ...
    $respuesta["mensaje"] = "Grupo agregado correctamente";
    $sentencia = null;
    $conexion = null;
    return $respuesta;
}