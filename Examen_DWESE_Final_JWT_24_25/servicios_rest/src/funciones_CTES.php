...
function horarioProfesor($id_profesor)
{
    try {
        $conexion = new PDO("mysql:host=" . SERVIDOR_BD . ";dbname=" . NOMBRE_BD, USUARIO_BD, CLAVE_BD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    } catch (PDOException $e) {
    ...
    try {
        $consulta = "select horario_lectivo.dia as dia, horario_lectivo.hora as hora, aulas.nombre as aula, grupos.nombre as grupo from horario_lectivo join aulas on horario_lectivo.aula = aulas.id_aula join grupos on horario_lectivo.grupo = grupos.id_grupo where usuario=?";
        $sentencia = $conexion->prepare($consulta);
    ...
    if ($sentencia->rowCount() > 0) {
        ...
        $respuesta["mensaje"] = "El usuario no se tiene horario en la BD";
...
function grupos()
{
    ...
    if ($sentencia->rowCount() > 0) {
        ...
        $respuesta["mensaje"] = "El grupo no se encuentra en la BD";
...
function horarioGrupo($id_grupo)
{
    ...
    try {
        $consulta = "select horario_lectivo.dia as dia, horario_lectivo.hora as hora, aulas.nombre as aula, usuarios.usuario as profe from horario_lectivo join aulas on horario_lectivo.aula = aulas.id_aula join usuarios on horario_lectivo.usuario = usuarios.id_usuario where grupo=?";
        $sentencia = $conexion->prepare($consulta);
    ...
    if ($sentencia->rowCount() > 0) {
        ...
        $respuesta["mensaje"] = "El grupo no tiene horario en la BD";
...
function profesores_grupo($dia, $hora, $id_grupo)
{
    ...
    try {
        $consulta = "select usuarios.id_usuario, usuarios.usuario, aulas.nombre as aula from usuarios join horario_lectivo on horario_lectivo.usuario = usuarios.id_usuario join aulas on horario_lectivo.aula = aulas.id_aula join grupos on horario_lectivo.grupo = grupos.id_grupo where dia=? and hora=? and id_grupo=?";
        $sentencia = $conexion->prepare($consulta);
    ...
    if ($sentencia->rowCount() > 0) {
        ...
        $respuesta["mensaje"] = "El grupo no tiene profesores en este horario en la BD";
...
function profesores_libres($dia, $hora, $id_grupo)
{
    ...
    try {
        $consulta = "SELECT usuarios.id_usuario, usuarios.usuario 
                        FROM usuarios 
                        WHERE usuarios.tipo <> 'admin' 
                        AND usuarios.id_usuario NOT IN (
                            SELECT usuarios.id_usuario
                            FROM usuarios 
                            JOIN horario_lectivo 
                                ON horario_lectivo.usuario = usuarios.id_usuario 
                            JOIN aulas 
                                ON horario_lectivo.aula = aulas.id_aula 
                            JOIN grupos 
                                ON horario_lectivo.grupo = grupos.id_grupo 
                            WHERE dia = ? AND hora = ? AND id_grupo = ?
                        )";
...
function quitar_profesor($dia, $hora, $id_grupo, $id_usuario)
{
    try {
        $conexion = new PDO("mysql:host=" . SERVIDOR_BD . ";dbname=" . NOMBRE_BD, USUARIO_BD, CLAVE_BD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    } catch (PDOException $e) {
